...
class join_existing extends join_edit {
  ...
  function form() {
    ...
    if (!isset($_COOKIE['atsphp_sid_user_cp'])) {
        header("Location: index.php?a=user_cpl");
    }
    ...
    if (isset($_COOKIE['atsphp_sid_user_cp'])) {
      ...
      $TMPL['owner_username'] = $DB->escape($data);
    }

    $TMPL['rnd_password'] = rand(1, 1000000);
    ...
    eval (PluginManager::getPluginManager ()->pluginHooks ('join_existing_fields'));
    ...
    eval (PluginManager::getPluginManager ()->pluginHooks ('join_existing_build_form'));

    if(!isset($altjoin)) {
		
		// csrf token at the end so plugins can not meddle with the template tag
		$TMPL['csrf_token'] = generate_csrf_token($TMPL['owner_username'], 'user_cp_join_csrf');
		
		
	    $TMPL['user_cp_content'] = $this->do_skin('join_form_existing');
    }
  ...
  function process() {
	...
	if (!isset($FORM['csrf_token']) || validate_csrf_token($FORM['csrf_token'], 'user_cp_join_csrf') === false) {
    ...
    $TMPL['ownerbox']    = isset($FORM['ownerbox']) ? $DB->escape($FORM['ownerbox'], 1) : '';
    if(empty($TMPL['ownerbox'])) {
    	$TMPL['owner'] = $TMPL['username'];
    } else {
    	$TMPL['owner'] = $TMPL['ownerbox'];    	
    }
    ...
    eval (PluginManager::getPluginManager ()->pluginHooks ('join_existing_process_form'));
    ...
    if ($this->check_ban('join')) {
      if ($this->check_input('join') && !in_array(0, $form_validate)) {
        ...
        $DB->query("INSERT INTO {$CONF['sql_prefix']}_sites 
                    ...
                    VALUES ('{$TMPL['username']}', '{$password}', '{$TMPL['url']}', '{$short_url}', '{$TMPL['title']}', '{$TMPL['description']}', '{$TMPL['category']}', '{$TMPL['banner_url']}', '{$TMPL['email']}', {$CONF['active_default']}, 0, '{$user_ip}','{$TMPL['owner']}'
        ...
        eval (PluginManager::getPluginManager ()->pluginHooks ('join_existing_insert_data'));
        ...
        if ($CONF['active_default']) {
          ...
          eval (PluginManager::getPluginManager ()->pluginHooks ('join_existing_finish'));

          $TMPL['user_cp_content'] = $this->do_skin('join_finish');
        }
        else {
          eval (PluginManager::getPluginManager ()->pluginHooks ('join_existing_finish_approve'));

          $TMPL['user_cp_content'] = $this->do_skin('join_finish_approve');
        }
      }
      else {
        $this->form();
