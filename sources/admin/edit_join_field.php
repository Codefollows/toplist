...
class edit_join_field extends base {
  public function __construct() {
    global $CONF, $DB, $FORM, $LNG, $TMPL;

    $TMPL['header'] = $LNG['a_edit_jf_header'];
    ...
    list($TMPL['id']) = $DB->fetch("SELECT id FROM {$CONF['sql_prefix']}_join_fields WHERE id = '{$id}'", __FILE__, __LINE__);
    if ($TMPL['id']) {
      ...
      $this->error($LNG['a_edit_jf_error'], 'admin');
    }
  }
  ...
  function form() {
    ...
    $row = $DB->fetch("SELECT * FROM {$CONF['sql_prefix']}_join_fields WHERE id = '{$TMPL['id']}'", __FILE__, __LINE__);
    if (isset($TMPL['count'])) {
        $field_choice_value_temp = $TMPL['field_choice_value'];
        $field_choice_text_temp = $TMPL['field_choice_text'];
    }

    $TMPL = array_merge($TMPL, $row);

    if (isset($TMPL['count'])) {
        $TMPL['field_choice_value'] = $field_choice_value_temp;
        $TMPL['field_choice_text'] = $field_choice_text_temp;
    }
    ...
    // Hide Field options when not needed
    $option_fields      = '';
    $option_text_fields = '';
    if($TMPL['field_type'] == 'textarea' || $TMPL['field_type'] == 'textbox'){
        $option_fields = ' style="display: none;"';
    } else {
        $option_text_fields = ' style="display: none;"';
    }
	...
	{
        $checked = isset($TMPL['show_join_edit']) && $TMPL['show_join_edit'] == $key ? 'checked="checked"' : '';
    ...
    $choice_value_array            = !empty($TMPL['field_choice_value']) ? explode(', ', $TMPL['field_choice_value']) : array();
    $choice_text_array             = !empty($TMPL['field_choice_text']) ? explode(', ', $TMPL['field_choice_text']) : array();
    ...
    $TMPL['count']                 = isset($TMPL['count']) ? $TMPL['count'] : count($choice_value_array);

    // Extra choice field inputs
    // In case of errors so user see same amount as before hitting submit
    $extra_choices = '';
    for ($i = 1; $i < $TMPL['count']; $i++) {
      $TMPL['error_field_choice_value_'.$i]       = isset($TMPL['error_field_choice_value_'.$i]) ? $TMPL['error_field_choice_value_'.$i] : '';
      $TMPL['error_style_field_choice_value_'.$i] = isset($TMPL['error_style_field_choice_value_'.$i]) ? $TMPL['error_style_field_choice_value_'.$i] : '';
...
<form action="index.php?a=admin&amp;b=edit_join_field&amp;id={$TMPL['id']}" method="post">

    <div id="tabs">
	    <ul>
		    <li><a href="#tabs-1">{$LNG['a_man_jf_tab1']}</a></li>
		    <li{$option_text_fields}><a href="#tabs-2">{$LNG['a_man_jf_tab2']}</a></li>
		    <li{$option_fields}><a href="#tabs-3">{$LNG['a_man_jf_tab3']}</a></li>
		    <li><a href="#tabs-4">{$LNG['a_man_jf_tab4']}</a></li>
	    </ul>

        <div id="tabs-1">
            <div><label for="field_id">{$LNG['a_man_jf_field_id']}</label>
                <input type="text" name="field_id" id="field_id" size="50" value="{$TMPL['field_id']}" disabled="disabled" />
	        </div>
    ...
    <div class="buttons">
        <button type="submit" name="submit" class="positive">{$LNG['a_man_jf_edit']}</button>
    </div>
...
EndHTML;
  ...
  function process() {
    ...
    {
        $DB->query("UPDATE {$CONF['sql_prefix']}_join_fields SET
                    label_text = '{$TMPL['label_text']}',
                    description = '{$TMPL['description']}',
                    field_type = '{$TMPL['field_type']}',
                    required = {$TMPL['required']},
                    field_text_requirements = '{$TMPL['field_text_requirements']}',
                    field_text_enable_length = '{$TMPL['field_text_enable_length']}',
                    field_text_length = '{$TMPL['field_text_length']}',
                    field_text_input_size = {$TMPL['field_text_input_size']},
                    field_choice_value = '{$TMPL['field_choice_value']}',
                    field_choice_text = '{$TMPL['field_choice_text']}',
                    display_location = '{$TMPL['display_location']}',
                    show_join_edit = {$TMPL['show_join_edit']}
                    WHERE id = {$TMPL['id']}", __FILE__, __LINE__);

				  

        $TMPL['admin_content'] = $LNG['a_man_jf_edit_success'];
