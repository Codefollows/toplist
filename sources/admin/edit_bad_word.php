...
<label for="word">{$LNG['a_man_bad_words_word']}</label>
<input type="text" name="word" id="word" size="20" value="{$TMPL['word']}" />

<label for="replacement">{$LNG['a_man_bad_words_replacement']}</label>
<input type="text" name="replacement" id="replacement" size="20" value="{$TMPL['replacement']}" />

<label for="matching">{$LNG['a_man_bad_words_matching']}</label>
...
<br />
<input name="submit" type="submit" value="{$LNG['a_edit_bad_word_header']}" class="positive" />
...
EndHTML;
  ...
  function process() {
    ...
    header("refresh:1; url={$TMPL['list_url']}/index.php?a=admin&b=edit_bad_word&id={$TMPL['id']}");	
  }
}
