...
<label for="string">{$LNG['a_man_ban_string']}</label>
<input type="text" name="string" id="string" size="20" value="{$TMPL['string']}" />

<label for="field">{$LNG['a_man_ban_field']}</label>
...
<br />
<input name="submit" type="submit" value="{$LNG['a_edit_ban_header']}" class="positive" />
...
EndHTML;
  ...
  function process() {
    ...
    header("refresh:1; url={$TMPL['list_url']}/index.php?a=admin&b=edit_ban&id={$TMPL['id']}");		
  }
}
