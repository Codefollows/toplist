...
class rate extends base {
  ...
  function form() {
	...
	if ($CONF['clean_url'] == 1 && preg_match('/\?/', $_SERVER['REQUEST_URI'])) 
	{
		...
		header("Location: {$CONF['list_url']}/review/{$TMPL['username']}/");
		exit;
	}
	...
	eval (PluginManager::getPluginManager ()->pluginHooks ('rate_build_form'));
  ...
  function process($ip, $ip_sql) {
    ...
    if (isset($FORM['review']) && $FORM['review']) {
	  ...
	  $TMPL['review'] = htmlspecialchars($review, ENT_QUOTES, "UTF-8");
	...
	eval (PluginManager::getPluginManager ()->pluginHooks ('rate_process_insert'));
	...
	eval (PluginManager::getPluginManager ()->pluginHooks ('rate_process_finish'));
