...
//BUTTON SETTINGS
include("{$CONF['path']}/button_config.php");

// For unknow reason, sometimes $location is not defined.
// So display static button as initialize. 
// If a different button type is triggered, it gets overwritten
$location = $CONF['static_button_url'];

// Check admin settings for need to connect to MySQL
if($CONF['count_pv'] == 1 || $CONF['rank_button'] == 1 || $CONF['stats_button'] == 1) {
...
// Check id for backwards compatability with ATS 4.x
...
} //End Check for pageview counting


// What button to display?
$rank_on_button = 0;
if (isset($_GET['buttontype']) && ($_GET['buttontype'] == 'rank' || $_GET['buttontype'] == 'stats')) {
...
if (isset($_GET['buttontype']) && $_GET['buttontype'] == 'rank' && !$rank_on_button) {
  $location = $CONF['default_rank_button'];
}


} 
// End Check for admin settings

if ((isset($_GET['buttontype']) && $_GET['buttontype'] == 'static') || !isset($_GET['buttontype'])) {
	$location = $CONF['static_button_url'];
}

elseif(isset($_GET['buttontype']) && $_GET['buttontype'] == 'text') {
	$location = $CONF['hidden_button_url'];
}
